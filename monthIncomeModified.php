...
<div  class="mainHeadDiv">Selling Report <?php echo(gmdate("Y-m", time() + 3600*($timezone+date("I")))) ?></div>
<hr width="100%">
<?php

$sql = "SELECT SUM(selling_price) FROM sell WHERE MONTH(date) = MONTH('".$date."');";
$result = $conn->query($sql);
$row = mysqli_fetch_assoc($result);


$sqlGetTodayIncome = "SELECT * FROM sell WHERE MONTH(date) = MONTH('2019-01-20')";
...
//			print_r($itemA);
			...
			foreach($itemA as $data){
				$qty = $co->getSumOfATable("sell","amount","WHERE MONTH(date) = MONTH('".$date."') AND item_id = ".$data['id'].";");
				if($qty != 0){
				?>
				<tr>
					...
					<td>
						<?php
							$tmpTotalAr = $co->select("sell","WHERE MONTH(date) = MONTH('".$date."') AND item_id = ".$data['id'].";");
							$tmpTotal = 0;
...
$sqlGetTodayIncome = "SELECT * FROM sell WHERE MONTH(date) = MONTH('".$date."') AND item_id = ".$rowItem['id'].";";
$resultGetTodayIncome = $conn->query($sqlGetTodayIncome);
