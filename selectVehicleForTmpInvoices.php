...
$lin = "ajaxCommonGetFromNet('".$url."sellSubmenu.php', 'mainStage');";

$basic = new basic;
$basic->menuBar("TMP Invoices - Select Vehicle",$lin);
$basic->modal();
...
while($row = mysqli_fetch_assoc($result)){
	...
	<div class="divList card-deck mb-3 text-center " style="background-color: white;color: black;border: 1px solid black">
        <div class="card mb-4 shadow-sm">
          ...
          <div class="card-body">
            ...
            <h1 class="card-title pricing-card-title"><?php echo($co->getNumRow("SELECT * FROM tmpInvoices WHERE invoiceN LIKE '%".$row['id']."-%' AND s = 1"))   ?>  <small class="text-muted">/ Invoices </small></h1>
            ...
            <button class="btn btn-primary btn-lg" onClick="getTmpInvoices(<?php echo($row['id']); ?>)">View</button>
	...
	<!---------------->
<!--
	<div class="tileButton" style="width: 800px;text-align: left;height: auto" onClick="getTmpInvoices(<?php echo($row['id']); ?>)">
			...
			<tr>
				...
				<td>
					...
					<table border="0">
						...
						<tr>
							<td>Total Cash</td>
							<td><?php echo(round($co->getSumOfATable("tmpInvoices","cash"," WHERE invoiceN LIKE '%".$row['id']."-%' AND s = 1"),2))?></td>
						</tr>
						<tr>
							<td>Total credit</td>
							<td><?php echo(round($co->getSumOfATable("tmpInvoices","credit"," WHERE invoiceN LIKE '%".$row['id']."-%' AND s = 1"),2))?></td>
						</tr>
					</table>
